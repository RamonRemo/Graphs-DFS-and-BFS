...
// We can represent the graph as a Map where each key (node) is an airport and the value (edges)
...
function bfs(start) {
    //Airports have many interconect rounds, so,
    //for our algorithm does not visit the same nodes we will use a set
    //where we will set the visited nodes that dont have our desire destination
    ...
    while (queue.length > 0) {
        ...
        for (const destination of destinations) {
            ...
            if (destination === 'BKK') console.log(`BFS found it!`);
...
// Depth-first Search (DFS) will go as far into the graph as possible 
// until it reaches a node without any children, at which point it backtracks
// and continues the process. The algorithm can be implemented with a recursive
// function that keeps track of previously visited nodes.
